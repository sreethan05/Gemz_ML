"""
Ultra-Fast High-Accuracy India Partition Inference Engine
...
class IndiaRecord:
    ...
    def __init__(self, eid: str, name: str, address: str):
        ...
        self.numbers = nums
        self.pins = frozenset(n for n in nums if len(n) == 6)  # Indian PIN codes are strictly 6 digits
...
def extract_india_blocking_keys(r: IndiaRecord) -> set:
    ...
    # 1. 6-digit Indian PIN codes (ultra sharp)
    for p in r.pins:
        keys.add(("p", p))
        if f2:
            keys.add(("pf", p, f2))

    # 2. Distinctive core tokens (length >= 4)
    for t in ctoks:
        if len(t) >= 4:
            keys.add(("n", t))

    # 3. Sorted signature (3 tokens)
    if len(ctoks) >= 2:
        sig = " ".join(sorted(ctoks)[:3])
        keys.add(("s", sig))

    # 4. House / Plot number + Name prefix
    for n in r.numbers:
        if f2 and len(n) >= 2:
            keys.add(("nf", n, f2))

    # 5. Distinctive address tokens (length >= 6)
...
def build_india_index(other_recs: list[IndiaRecord], max_bucket: int = 50):
    """
    Ultra-compact, high-precision inverted index with max_bucket=50.
    Drops common non-distinctive buckets in O(1) time. Memory < 50MB.
    """
...
def query_india_candidates(r: IndiaRecord, index: dict, max_candidates: int = 15) -> list[int]:
    hit_counts = Counter()
    for k in extract_india_blocking_keys(r):
        b = index.get(k)
        if b:
...
def load_india_compact(path: Path) -> list[IndiaRecord]:
    recs = []
    for chunk in pd.read_csv(path, sep="\t", dtype=str, keep_default_na=False, chunksize=400000, quoting=csv.QUOTE_NONE):
        for r in chunk.itertuples(index=False):
            recs.append(IndiaRecord(str(r.entity_id).strip(), r.business_name, r.business_address))
    return recs
...
def main():
    ...
    log("=" * 70)
    log("ULTRA-FAST STREAMING INFERENCE — INDIA PARTITION")
    log("=" * 70)
    ...
    # 2. Verify France and US lines in output files
    existing_eids = set()
    ...
    log(f"Existing completed entities in matching_results.tsv: {len(existing_eids):,} (France + US complete)")
    if len(existing_eids) != 922558:
        log(f"WARNING: Expected exactly 922,558 completed entities before India, got {len(existing_eids):,}")

    t_start = time.time()

    # 3. Load India Partition
    log("[1/3] Loading India S1 and Other records (Optimized Compact Parsing)...")
    s1_india = load_india_compact(part_dir / "india_s1.tsv")
    other_india = load_india_compact(part_dir / "india_other.tsv")
    log(f"Loaded |S1|={len(s1_india):,}, |Other|={len(other_india):,} in {(time.time() - t_start):.1f}s")

    # 4. Build Sharp Bounded Index
    t_idx = time.time()
    log("[2/3] Building Sharp Bounded Inverted Index (max_bucket=50)...")
    idx_india = build_india_index(other_india, max_bucket=50)
    log(f"Inverted index built with {len(idx_india):,} active keys in {(time.time() - t_idx):.1f}s")

    # 5. Stream Inference
    log("[3/3] Streaming inference across India entities...")
    BATCH_ENTITIES = 20000
    n_s1 = len(s1_india)
    ...
    with open(m_path, "a", encoding="utf-8") as fm, open(c_path, "a", encoding="utf-8") as fc:
        for b_start in tqdm(range(0, n_s1, BATCH_ENTITIES), desc="Scoring INDIA"):
            ...
            for r in batch_s1:
                cands = query_india_candidates(r, idx_india, max_candidates=15)
                if not cands:
                    fm.write(f"{r.id}\t\n")
                    fc.write(f"{r.id}\t\n")
                    total_singletons += 1
    ...
    log(f"India scoring complete in {total_min:.2f} mins! Matches: {total_matches:,}, Singletons: {total_singletons:,}")
    ...
    log(f"Files ready for direct upload: {m_path.resolve()} and {c_path.resolve()}")
    return 0
